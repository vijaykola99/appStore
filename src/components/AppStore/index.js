...
class AppStore extends Component {
  ...
  searchData = () => {
    ...
    const searchList = appsList.filter(eachList =>
      eachList.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchList
  }

  getFilteredApps = search => {
    const {activeTab} = this.state
    const filteredApps = search.filter(
  ...
  render() {
    ...
    const filteredApps = this.getFilteredApps(search)
    return (
      <div className="app-background">
        ...
        <div className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              changeActiveTab={this.onChangeTabId}
              active={eachTab.tabId === activeTab}
              key={eachTab.tabId}
              data={eachTab}
            />
          ))}
